...
## Lessons learned
- *What slowed me down:* Using nmap directly on port `8080` did not work because I used the `-Pn` flag when running it against port `8080`. I dropped the `-Pn` flag and it worked but the problem is that it was just a timing thing. 
